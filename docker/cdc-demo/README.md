...
> `docker build -f docker/cdc-demo/Dockerfile.sidecar -t cassandra-sidecar:dev .`
> then update `docker-compose.yml` to use `image: cassandra-sidecar:dev` instead of
> the `build:` block, and run `docker compose up -d`.

### Step 2 — Watch startup progress

```bash
docker compose logs -f cassandra cassandra-init sidecar
```

Wait until you see `CDC iterators started successfully` in the sidecar logs.
You can also check sidecar health directly:
...
1. Click the **local** cluster in the sidebar.
2. Click **Topics** → select **cdc-mutations**.
3. Click the **Messages** tab to see one Avro-encoded message per Cassandra mutation.
...
The `LOCAL_JMX=no` env var enables remote JMX on the Cassandra container.
If the sidecar still can't connect, check the JVM flags:
